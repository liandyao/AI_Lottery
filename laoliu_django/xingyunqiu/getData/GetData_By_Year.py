...
data = []  # 用于存储提取的数据
# 循环从 2004 年到 2024 年
def getData(year): # 传入 2024
    print(f"正在获取年份: {year} 的数据...")
    params = {
        "startqi": "",  # 空字符串
        "endqi": "",  # 空字符串
        "year": str(year),  # 当前年份
    ...
    # 导出为 CSV 文件
    with open(f'../data/lottery_results_{year}.csv', 'w', newline='', encoding='utf-8') as csvfile:
        ...
        print(f"数据已成功导出为 lottery_results_{year}.csv")

if __name__ == '__main__':
    getData(2025)
    print("数据已成功导出为 lottery_results.csv")